   ...
   .then(data =>{
    ...
    getAllCourses()
     .then(courseData =>{ 
      ...
      let course = _.find(courseData.courses, c => Number(c.id) === Number(mockExam.courseId));
      ...
      if(Array.isArray(mockExam.tests) ) {
       _.forEach(mockExam.tests, t=>{
        subjectsSelected[t.subjectId]=true;
       });
       activeTest = mockExam.tests[0];       
       if( course && Array.isArray(course.subjects) && activeTest){
        activeSubject = _.find(course.subjects, s => Number(s.id) === Number(activeTest.subjectId));
       }       
      }
   
      this.setState({mockExam:data.mockExam,
...
doExamRefresh =()=>{
 if(this.state.mockExam && this.state.mockExam.id){
  getMockExamById(this.state.mockExam.id)
   .then(examData =>{
    let currentMockExam = this.state.mockExam;
    currentMockExam['version']=examData.mockExam.version;
    currentMockExam['tests']=examData.mockExam.tests;
    if(Array.isArray(currentMockExam.tests) && this.state.activeTest ){
     currentMockExam.tests.push(this.state.activeTest);
    }
    this.setState({mockExam:currentMockExam, isError:false, message:''});
   })
   .catch(error =>{
    console.log('Error > ', error);
    this.setState({isError:true,message:'Error while refreshing the exam'});
   });
 }
}
    ...
    <div className="text-center">      
     {this.state.message?
      <div className={"text-style alert "+ (this.state.isError?' alert-danger':'alert-success') }>
       {this.state.message}
       {this.state.isError && <button className="btn btn-primary" onClick={this.doExamRefresh}>Reload The Exam</button>}
      </div>:''}
    </div>
   </div>
        ...
        <div className="row ">
         <form className="form-inline" autoComplete ="off">
          ...
          <div className="input-group">
           <label htmlFor="testName" alt={'Hello World'}>Test Name:</label>
           <input className="form-control" readOnly
            data-toggle="tooltip" data-placement="bottom" 
            title={(this.state.mockExam.name?this.state.mockExam.name+'-':'')+(this.state.activeSubject?this.state.activeSubject.name:'')}
